...
* create a function to convert "TN_CONC_inlake" (mg/l) to trophic state
* 

```{r analysis, include=FALSE, echo=FALSE, eval=TRUE}
ec02<-read_sas('data/mrb1mrb2_massbalance_2002base.sas7bdat');dim(ec02) #41566    42
ec25<-read_sas('data/mrb1mrb2_massbalance_2025.sas7bdat');dim(ec25) #41566    42
...
assignTS<-function(x){  #x is a vector of TN conventration in mg/l
  ...
  ts<-cut(tsi,thresholds,include.lowest=TRUE,labels=c('oligo','meso','eu','hyper')) #assign the TS
  return(ts)
}
...
ts02<-assignTS(ec02$TN_CONC_inlake)
ts25<-assignTS(ec25$TN_CONC_inlake)
