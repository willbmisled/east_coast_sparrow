...
group_by(ec_ts,ts02)%>%count(ts25)
...
a<-as.data.frame(table(ec_ts$ts02))%>%rename(ts2002=Freq)
b<-as.data.frame(table(ec_ts$ts25))%>%rename(ts2025=Freq)
ts_tbl<-inner_join(a,b)%>%mutate(ts_chg=ts2025-ts2002)%>%rename(ts=Var1)


```

* These nitrogen estimates were converted to Trophic State categories and compared
* All of the changes in tro
